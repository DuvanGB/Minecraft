...
public class WorldGenerator : MonoBehaviour
{
    ...
    private void GenerateWorld() {  // The seed that will make the map random is generated
        _seed = UnityEngine.Random.Range(0.0f, 1000.0f); // Mesh of chung
        for (int i = 0; i < N_CHUNKS_X; ++i) {
            for (int j = 0; j < N_CHUNKS_Z; ++j) {
                GenerateChunk(i, j); // For each chung a patch is generated using the perlin noise combined with the map seed
            }
        }
    }
    private void GenerateChunk(int cx, int cz) { 
        ...
        for (int x = 0; x < N_PATCHES_X; ++x)
        {
            for (int y = 0; y < N_PATCHES_Y; ++y)
            {
                ...
                {   // Curve definided in the inspector
                    ...
                    float density = heightDensityCurve.Evaluate(heightPerc); // Chung density depending on its height
